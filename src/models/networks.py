...
class TestSERv4(nn.Module):
    def __init__(
        self,
        cfg: Config,
        device: str = "cpu",
    ):
        super(TestSERv4, self).__init__()
        # Text module
        self.text_encoder = build_text_encoder(cfg.text_encoder_type)
        self.text_encoder.to(device)
        # Freeze/Unfreeze the text module
        ...
        self.text_linear = nn.Linear(cfg.text_encoder_dim, cfg.text_encoder_dim)
        self.text_layer_norm = nn.LayerNorm(cfg.text_encoder_dim)


        self.text_attention_2 = nn.MultiheadAttention(
            embed_dim=cfg.text_encoder_dim,
            num_heads=cfg.num_attention_head,
            dropout=cfg.dropout,
            batch_first=True,
        )
        self.text_linear_2 = nn.Linear(cfg.text_encoder_dim, cfg.fusion_dim)
        self.text_layer_norm_2 = nn.LayerNorm(cfg.fusion_dim)
        ...
        self.audio_linear = nn.Linear(cfg.audio_encoder_dim, cfg.audio_encoder_dim)
        self.audio_layer_norm = nn.LayerNorm(cfg.audio_encoder_dim)

        

        self.audio_attention_2 = nn.MultiheadAttention(
            embed_dim=cfg.audio_encoder_dim,
            num_heads=cfg.num_attention_head,
            dropout=cfg.dropout,
            batch_first=True,
        )
        self.audio_linear_2 = nn.Linear(cfg.audio_encoder_dim, cfg.fusion_dim)
        self.audio_layer_norm_2 = nn.LayerNorm(cfg.fusion_dim)
    ...
    def forward(
        ...
        # Text cross attenttion Q audio , K and V text
        text_attention, text_attn_output_weights = self.text_attention(
            audio_embeddings,
            text_embeddings,
            text_embeddings,
        ...
        # Text cross attention Q audio, K and V text
        text_attention, _ = self.text_attention_2(
                audio_norm,
                text_norm,
                text_norm,
                average_attn_weights=False,
                )
        text_linear = self.text_linear_2(text_attention)
        text_norm = self.text_layer_norm_2(text_linear)

        # Audio cross attention Q text, K and V audio
        audio_attention, _ = self.audio_attention_2(
                text_norm,
                audio_norm,
                audio_norm,
                average_attn_weights=False,
                )
        audio_linear = self.audio_linear_2(audio_attention)
        audio_norm = self.audio_layer_norm_2(audio_linear)
